...
TEST(BridgesTest, Chain_AllEdgesAreBridges) {
    ...
    auto bridges = graph.bridges(node1);

    // Expected bridges
    ASSERT_TRUE(has_bridge(bridges, node1, node2));
    ASSERT_TRUE(has_bridge(bridges, node2, node3));
}
...
TEST(BridgesTest, TrianglePlusTail_OnlyTailIsBridge) {
    ...
    ASSERT_TRUE(has_bridge(bridges, node3, node4));

    // All other edges should NOT be bridges
    ASSERT_FALSE(has_bridge(bridges, node1, node2));
    ASSERT_FALSE(has_bridge(bridges, node2, node3));
    ASSERT_FALSE(has_bridge(bridges, node1, node3));
}
